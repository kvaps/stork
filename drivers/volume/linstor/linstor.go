...
func (l *linstor) InspectVolume(volumeID string) (*storkvolume.Info, error) {
	logrus.Infof("called: InspectVolume(%s)", volumeID)
	rd, err := l.cli.ResourceDefinitions.Get(context.TODO(), volumeID)
	if err != nil {
		return nil, fmt.Errorf("failed to get resource definition: %w", err)
	}

	resources, err := l.cli.Resources.GetAll(context.TODO(), volumeID)
	if err != nil {
		return nil, fmt.Errorf("failed to get resources: %w", err)
	}

	nodes := make([]string, len(resources))
	for i := range resources {
		nodes[i] = resources[i].NodeName
	}

	vd, err := l.cli.ResourceDefinitions.GetVolumeDefinition(context.TODO(), volumeID, 0)
	if err != nil {
...
func (l *linstor) GetNodes() ([]*storkvolume.NodeInfo, error) {
	logrus.Infof("called: GetNodes()")
	nodes, err := l.cli.Nodes.GetAll(context.TODO())
	if err != nil {
...
func (l *linstor) GetPodVolumes(podSpec *v1.PodSpec, namespace string) ([]*storkvolume.Info, error) {
	logrus.Infof("called: GetPodVolumes(%s, %s)", podSpec.Containers[0].Name, namespace)
...
func (l *linstor) GetVolumeClaimTemplates(templates []v1.PersistentVolumeClaim) ([]v1.PersistentVolumeClaim, error) {
	logrus.Infof("called: GetVolumeClaimTemplates([%d pvcs])", len(templates))
...
func (l *linstor) OwnsPVC(pvc *v1.PersistentVolumeClaim) bool {
	logrus.Infof("called: OwnsPVC(%s)", pvc.ObjectMeta.Name)
	...
	if provisioner == "" {
		...
		logrus.Infof("No information in PVC, try PV?")
		return l.OwnsPV(pv)
	}
...
func (l *linstor) OwnsPV(pv *v1.PersistentVolume) bool {
	logrus.Infof("called: OwnsPV(%s)", pv.ObjectMeta.Name)
	provisioner := ""
...
func (l *linstor) GetSnapshotPlugin() snapshotVolume.Plugin {
	logrus.Infof("called: GetSnapshotPlugin()")
	return nil
}

func (l *linstor) GetSnapshotType(snap *snapv1.VolumeSnapshot) (string, error) {
	logrus.Infof("called: GetSnapshotType(%s)", snap.String())
	return "", &errors.ErrNotImplemented{}
}

func (l *linstor) Stop() error {
	logrus.Infof("called: Stop()")
	close(l.stopChannel)
	return nil
}
...
func (l *linstor) GetClusterID() (string, error) {
	logrus.Infof("called: GetClusterID()")
	props, err := l.cli.Nodes.GetControllerProps(context.TODO())
	if err != nil {
	...
	if id == "" {
		id = randString(16)
		logrus.Infof("linstor: no cluster ID found, generating new (%s)", id)
		err := l.cli.Nodes.ModifyController(context.TODO(), lclient.GenericPropsModify{
...
func init() {
	...
	err := l.Init(nil)
	if err != nil {
		logrus.Errorf("Error init'ing linstor driver: %v", err)
		return
	}
